function [x,x_debias,objective,times,debias_start,mses,max_svd] = ...
         solveRegularise(y,A,tau,varargin)
%
% Usage:
% [x,x_debias,objective,times,debias_start,mses] = solveRegularise(y,A,tau,varargin)
%
...
% is known. 
% 
% For further details about the solveRegularise algorithm, see the paper:
%
...
while cont_outer
    ...
    while for_ever
        % IST estimate
        x = psi_function(xm1 + iter_rate*grad/max_svd,tau/max_svd);
        if (IST_iters >= 2) | ( TwIST_iters ~= 0)
            ...
            if (f > prev_f) & (enforceMonotone)
                ...
                TwIST_iters = TwIST_iters+1; % solveRegularise iterations
                IST_iters = 0;
                x = xm2;
